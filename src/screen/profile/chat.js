...
function Chat({data,sendMessage,firebaseMessages,getMessages,readMessages,clearChat,type,getMobileToken}) {
  const {sId,rId,sName,sImage,rec}=data
  ...
  useEffect(()=>{
    getSize()
    const {sId,rId,sName,sImage,rec}=data
    getMessages(data.sId,data.rId,lastDate ,()=>readMessages(sId,rId,sName,type=="client"?rec.user_name:rec.client_name)).then(res=>{
  ...
  function getSize(){
    ...
    firestore().collection('chat').doc("conversation").collection(mergeId(data.sId,data.rId)).onSnapshot(data=>{
      setSize(data.size)
    })
  }
  ...
  const onSend = useCallback((messages = [],imgUrl,fileUrl,extension) => {
    ...
    if(imgUrl || messages[0].text || fileUrl){
      ...
      sendMessage(messages,messages[0].user._id,{id:rId,name:(type=="client"?rec.user_name:rec.client_name),url:(type=="client"?managerUrl:clientUrl)+(type=="client"?rec.user_image:rec.client_image),mobileToken:rec.mobile_token},messages[0].user.name,messages[0].user.avatar).then(async()=>{
        
        //send notification
        getMobileToken(rId,messages[0].user.name,messages[0].text)
  ...
  return (
    <>
        ...
        user={{
          _id: data.sId,
          name:data.sName,
          avatar:(type=="client"?clientUrl:managerUrl)+data.sImage,
        }}
        renderLoading={()=>(
